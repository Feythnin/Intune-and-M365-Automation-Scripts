...
Describe 'Script Validation - <Name>' -ForEach $scripts {
    ...
    It 'checks for a service connection (Get-MgContext or Get-OrganizationConfig)' {
        ($scriptContent -match 'Get-MgContext') -or ($scriptContent -match 'Get-OrganizationConfig') | Should -BeTrue
    }
